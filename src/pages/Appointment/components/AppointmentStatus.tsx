...
const AppointmentStatus = ({ status }: Props) => {
  ...
  switch (status.toLowerCase()) {
    case 'waitingapproval':
      ...
      break;
    case 'confirm':
      current = <Chip label={t('appointment_status.confirm')} size="small" className="!text-green-600 !bg-green-200 !rounded-md min-w-[120px]" />;
      break;
    case 'reject':
      current = <Chip label={t('appointment_status.reject')} size="small" className="!text-orange-600 !bg-orange-200 !rounded-md min-w-[120px]" />;
      break;
    case 'completed':
      current = <Chip label={t('appointment_status.completed')} size="small" className="!text-purple-600 !bg-purple-200 !rounded-md min-w-[120px]" />;
      break;
    case 'absent':
